...
class pdf_azur extends ModelePDFPropales
{
	...
	public function __construct($db)
	{
		...
		$this->postotalht = 174;
		if (!empty($conf->global->MAIN_GENERATE_PROPAL_WITHOUT_VAT) || !empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT) || !empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_COLUMN)) {
	...
	protected function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs, $outputlangsbis = null)
	{
		...
		$hasTax = $total_ht != $total_ttc;
		
		$pdf->SetFillColor(255, 255, 255);
		$pdf->SetXY($col1x, $tab2_top + 0);
		$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities($hasTax ? "TotalHT" : "Total"), 0, 'L', 1);
		...
		$this->atleastoneratenotnull = 0;
		if ($hasTax && empty($conf->global->MAIN_GENERATE_PROPAL_WITHOUT_VAT) && empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT)) {
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		*/
		...
		if ($showaddress) {
			...
			} else if ($object->user_author_id > 0) {
			    $object->fetch_user($object->user_author_id);
			}
			
			$carac_emetteur .= pdf_build_address($outputlangs, $this->emetteur, $object->thirdparty, '', 0, 'source', $object, $object->user);
