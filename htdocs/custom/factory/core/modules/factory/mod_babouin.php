...
class mod_babouin extends ModeleNumRefFactory
{
	...
	function info()
	{
		...
		$babouin_mask="";
		if (!empty($conf->global->FACTORY_BABOUIN_MASK))
			$babouin_mask=$conf->global->FACTORY_BABOUIN_MASK;
		$texte.= '<tr><td>'.$langs->trans("Mask").':</td>';
		$texte.= '<td align="right">';
		$texte.= $form->textwithpicto(
				'<input type="text" class="flat" size="24" name="maskvalue" value="'.$babouin_mask.'">', 
				$tooltip, 1, 1
		).'</td>';
