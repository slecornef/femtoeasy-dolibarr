...
abstract class ModeleFactory extends ModelePDFFactory
{}
 ...
 */
function factory_create($db, $object, $modele, $outputlangs, $hidedetails=0, $hidedesc=0, $hideref=0, $hookmanager=false)
{
	...
	if (! dol_strlen($modele)) {
		if ($conf->global->FACTORY_ADDON_PDF) 
			$modele = $conf->global->FACTORY_ADDON_PDF;
		else
			$modele = 'capucin';
	}
	...
	foreach ($dirmodels as $reldir) {
		foreach (array('doc', 'pdf') as $prefix) {
			...
			$file=dol_buildpath($reldir."core/modules/factory/doc/".$file, 0);
			if (file_exists($file)) {
				$filefound=1;
	...
	if ($filefound) {
		...
		if ($obj->write_file(
						...
						$hidedetails, $hidedesc, $hideref, $hookmanager
		) > 0) {
