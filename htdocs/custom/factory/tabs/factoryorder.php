...
/**
 *  \file	   htdocs/factory/tabs/factoryorder.php
 *  \ingroup	product
 *  \brief	  Page de création des OF depuis une commande client
 */
 ...
 * Actions
 */
// build product on each line of order
if ($action == 'createof' ) {
	...
	$factory->origin="commande";
	$factory->origin_id=$id;
	// on boucle sur la liste des produits à fabriquer
	foreach ($object->lines as $line) {
		...
		if ($factory->is_FactoryProduct($line->fk_product) > 0) {
			...
			if ($factory->qty_planned > 0) {
				...
				// on reset le tableau des sous produits
				$factory->sousprods = array();
				// on transmet en id le produit que l'on veut fabriquer
...
llxHeader("", $langs->trans("FactoryOrder"));

$head = commande_prepare_head($object);

dol_fiche_head($head, 'factory', $langs->trans("CustomerOrder"), -1, 'order');
...
print '<form action="factoryorder.php?id='.$id.'" method="post">';
...
$sql .= " WHERE el.fk_source= ".$id." AND el.sourcetype='commande'";
